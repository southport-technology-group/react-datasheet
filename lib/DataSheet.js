  ...
  _createClass(DataSheet, [{
      ...
      if (isEditing) {
        ...
        if (currentCell && currentCell.component && !currentCell.forceComponent) {
          ...
          if (keyCode === _keys.ENTER_KEY) {
            func = function func() {
              return _this3.handleNavigate(e, { i: offset, j: 0 }, true);
            };
    ...
    value: function onMouseDown(i, j, e) {
      ...
      this._setState({
        ...
        start: e.shiftKey | e.fromCopydownHandle ? this.state.start : { i: i, j: j },
        end: e.fromCopydownHandle ? this.state.end : { i: i, j: j },
        editing: editing,
    ...
    value: function onMouseOver(i, j) {
      ...
      function calculateCopydownTarget(coords) {
        ...
        var smallestI = Math.min(start.i, end.i);
        var largestI = Math.max(start.i, end.i);
        var smallestJ = Math.min(start.j, end.j);
        var largestJ = Math.max(start.j, end.j);

        var iWithinSelection = i >= smallestI && i <= largestI;
        var jWithinSelection = j >= smallestJ && j <= largestJ;

        // If dragging within selection, target is {}
        if (iWithinSelection && jWithinSelection) return {};

        // Otherwise get the delta...
        var deltaI = i < smallestI ? i - smallestI : i - largestI;
        var deltaJ = j < smallestJ ? j - smallestJ : j - largestJ;

        if (iWithinSelection) return { j: deltaJ };
        if (jWithinSelection) return { i: deltaI };else {
          var _target = Math.abs(deltaI) >= Math.abs(deltaJ) ? { i: deltaI } : { j: deltaJ };
          return _target;
        }
      }
      ...
      if (this.state.copydownDragging) {
        ...
        var _target2 = calculateCopydownTarget({ i: i, j: j });
        this._setState({ copydownTarget: _target2 });
      }
    }
  }, {
    key: 'onMouseUp',
    value: function onMouseUp() {
      var _state2 = this.state,
          copydownDragging = _state2.copydownDragging,
          copydownTarget = _state2.copydownTarget,
          start = _state2.start;
      ...
      if (copydownDragging && !isEmpty(copydownTarget)) {
        ...
        var isVertical = !!copydownTarget.i;
        var extendedSide = isVertical ? 'row' : 'col';
        var constantSide = isVertical ? 'col' : 'row';
        var constantCoord = isVertical ? j : i;
        var changeTemplate = {};
        changeTemplate[constantSide] = constantCoord;
        changeTemplate.value = this.isEditing(i, j) ? this.state.editValue : data[i][j].value;

        // Number of rows/cols copied into
        var offset = copydownTarget.i | copydownTarget.j;

        // Coordinates of first and last col
        var startingPoint = isVertical ? offset > 0 ? i + 1 : i + offset : offset > 0 ? j + 1 : j + offset;
        var max = isVertical ? offset > 0 ? i + offset : i - 1 : offset > 0 ? j + offset : j - 1;

        for (var x = startingPoint; x <= max; x++) {
          var change = _extends({}, changeTemplate);
    ...
    key: 'isBottomRightSelected',
    value: function isBottomRightSelected(i, j) {
      var _getState9 = this.getState(),
          start = _getState9.start,
          end = _getState9.end;

      var largestI = start.i > end.i ? start.i : end.i;
      var largestJ = start.j > end.j ? start.j : end.j;
      return i === largestI && j === largestJ;
    }
  }, {
    key: 'isSingleSelected',
    value: function isSingleSelected(i, j) {
    ...
    key: 'isCopydownTargeted',
    value: function isCopydownTargeted(i, j) {
      var _state4 = this.state,
          target = _state4.copydownTarget,
          start = _state4.start,
          end = _state4.end;
      ...
      if (target.i) {
        ...
        var jWithinSelection = j >= smallestJ && j <= largestJ;
        var iWithinTarget = target.i > 0 ? i > largestI && i <= largestI + target.i : i < smallestI && i >= smallestI + target.i;

        return jWithinSelection && iWithinTarget;
      } else if (target.j) {
        // Within selection i and in target j return true
        var iWithinSelection = i >= smallestI && i <= largestI;
        var jWithinTarget = target.j > 0 ? j > largestJ && j <= largestJ + target.j : j < smallestJ && j >= smallestJ + target.j;
    ...
    value: function render() {
      ...
      var _state5 = this.state,
          forceEdit = _state5.forceEdit,
          selecting = _state5.selecting,
          copydownDragging = _state5.copydownDragging;
            ...
            return _react2.default.createElement(
              ...
              row.map(function (cell, j) {
                ...
                return _react2.default.createElement(_DataCell2.default, _extends({
                  ...
                  copydownTargeted: _this5.isCopydownTargeted(i, j),
                  selecting: selecting,
                  copydownDragging: copydownDragging,
                  lastSelected: _this5.isSingleSelected(i, j) // Replace with isBottomRightSelected to enable multicell copydown
                  , editing: isEditing,
